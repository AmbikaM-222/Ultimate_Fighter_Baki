...
void MAXreg_wr(BYTE reg, BYTE val) {
   ...
   //write val via SPI
    BYTE buffer_send [2] = {reg+2, val};




int SPI = XSpi_Transfer(&SpiInstance, buffer_send, NULL, 2);
if (SPI != 0){
    xil_printf ("error");
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
   ...
   //select MAX3421E (may not be necessary if you are using SPI peripheral)
    ...
    for (int j =0; j < nbytes; j++) {
       buffer_send [j+1] = data [j];
    }
  int SPI = XSpi_Transfer(&SpiInstance, buffer_send, NULL, nbytes + 1);
if (SPI != 0){
    xil_printf ("error");
}
...
BYTE MAXreg_rd(BYTE reg) {
   ...
   //select MAX3421E (
    XSpi_SetSlaveSelect(&SpiInstance, 1);
   //write reg via SPI
    BYTE buffer_send[2]={reg, 0};
   //read val via SPI
    long SPI = XSpi_Transfer(&SpiInstance, buffer_send, buffer_send, 2);
if (SPI != 0){
    xil_printf ("error");
}
   //read return code from SPI peripheral
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
   ...
   //read data[n] from SPI, where n goes from 0 to nbytes-1
    buffer_send [0] = reg;
    long SPI = XSpi_Transfer(&SpiInstance, buffer_send, buffer_send, nbytes + 1);
  if (SPI != 0){
      xil_printf ("error");
  }
